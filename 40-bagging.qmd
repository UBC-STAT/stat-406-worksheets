...
```{r bag10, fig.width=5, fig.height=5, message=FALSE, warning=TRUE, echo=TRUE}
N <- 10
myps <- array(NA, dim = c(nrow(dat.te), N))
n.tr <- nrow(dat.tr)
set.seed(123)
...
```
or $N = 100$ trees? 
```{r bag100, fig.width=5, fig.height=5, message=FALSE, warning=TRUE, echo=TRUE}
N <- 100
myps <- array(NA, dim = c(nrow(dat.te), N))
n.tr <- nrow(dat.tr)
set.seed(123)
...
```
or $N = 1000$ trees? 
```{r bag1000, fig.width=5, fig.height=5, message=FALSE, warning=TRUE, echo=TRUE}
N <- 1000
myps <- array(NA, dim = c(nrow(dat.te), N))
n.tr <- nrow(dat.tr)
set.seed(123)
...
estimated MSPEs:
```{r bagsplines4, fig.width=5, fig.height=5, message=FALSE, warning=FALSE}
N <- 100
myps <- matrix(NA, nrow(lid.te), N)
set.seed(123)
n.tr <- nrow(lid.tr)
